...
import { Search, ShieldAlert, CheckCircle2, ShieldCheck } from 'lucide-react';
    ...
    <div className="bg-white min-h-screen py-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-4xl font-bold text-navy mb-4 italic tracking-tight uppercase">Certificate Verification</h1>
          <p className="text-navy/50 max-w-2xl mx-auto italic">Verify the core training completion and competency standing for GCMWC certificate holders.</p>
        </div>

        <div className="bg-beige/40 p-10 border border-navy/5 shadow-inner mb-16 constellation-motif text-center">
          <div className="relative max-w-2xl mx-auto">
            <input 
              ...
              placeholder="Search Name or Certificate ID (e.g. C-1004)"
          ...
          <p className="mt-4 text-[10px] text-navy/40 font-bold uppercase tracking-widest">
            Results are pulled directly from the GCMWC global ledger.
          </p>
        </div>
            ...
            {results.length > 0 ? (
              results.map((entry) => (
                <div key={entry.id} className="border-2 border-beige p-10 shadow-sm hover:shadow-lg transition-all bg-white relative overflow-hidden group">
                  <div className="absolute left-0 top-0 bottom-0 w-2 bg-navy"></div>
                  
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="space-y-3">
                      <div className="flex items-center space-x-4">
                        <h3 className="text-2xl font-bold text-navy italic">{entry.name}</h3>
                        <span className="flex items-center space-x-2 px-4 py-1.5 border border-teal/30 bg-teal/5 text-forest text-[10px] font-bold uppercase tracking-widest rounded-sm">
                    ...
                    <div className="flex flex-col text-[10px] font-bold uppercase tracking-[0.25em] text-navy/30">
              ...
              <div className="text-center py-24 bg-beige/10 border-4 border-dashed border-beige">
                ...
                <p className="text-navy/40 font-bold uppercase tracking-[0.3em]">No matching records found in verification system.</p>
              </div>
            )}
          </div>
        )}
